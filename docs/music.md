...
`app/music/tracks.ts` contains 10 specific recording IDs. Each was checked against its individual Spotify embed's `isExplicit` value. The verification record is in `music-recordings.json`. This is provider metadata verification, not a guarantee that a song has no mature themes. Do not substitute an artist feed, an explicit album edition, a cover or a third-party upload.

Normal tracks share the regular selection pool. The first five page visits on a browser exclude the rare recording from both shuffle and manual selection. A local-storage counter unlocks it on visit six; reopening the intro in the same page does not increment the count. Unavailable storage keeps it locked. Clearing browser data restarts the count.

Once unlocked, the original cast recording of “Fukashigi no Karte” has a 1% random selection probability, unless it is the current track (immediate repeats are excluded). Users may also choose it directly. Shuffle never immediately repeats the current recording.
